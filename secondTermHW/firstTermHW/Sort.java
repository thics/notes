...
public class Sort {
  ...
  public long bubble() {
    ...
    int sortNums = 1;
    while( (list.length - sortNums - 1) >= 0) {
      for(int i = 0; i < size() - sortNums; i++) {
        if(list[i] > list[i + 1]) {
          swap(i, i + 1);
          swaps++;
        }
  ...
  public long selection() {
    ...
    while(insertPos < list.length) {
      //Keep comparing until you find an end, and swap it with the current index term
      swapPos = insertPos;
      for(int i = insertPos; i < list.length; i++) {
        if(list[i] <= list[swapPos])
          swapPos = i;
        comps++;
      }

      if( insertPos != swapPos) 
        swap(insertPos, swapPos);
      insertPos++;
    }
    return System.currentTimeMillis() - time;
  }
  ...
  public long insertion() {
    ...
    for(int i = 1; i < list.length; i++) {
      int value = list[i];
      int check = i - 1;
      while(value < list[check] && check > 0) {
        //Shifting all values over and getting the insertion index value
